 ...
 * @param length
 */
export const chars = (length: number) =>
  tString().where((s) => s.length === length, `expected a string of length ${length}`);

/**
 * Matches a string having a minimum of `min` characters.
 * @param min
 */
export const charsMin = (min: number) =>
  tString().where((s) => s.length >= min, `expected a string of minimum length ${min}`);

/**
 * Matches a string having a maximum of `max` characters.
 * @param max
 */
export const charsMax = (max: number) =>
  tString().where((s) => s.length <= max, `expected a string of maximum length ${max}`);
 ...
 * @param max
 */
export const charsRange = (min: number, max: number) =>
  tString().where((s) => s.length >= min && s.length <= max, `expected a string of between ${min} and ${max} in length`);
...
export const matches = (pattern: RegExp) =>
  tString().where((s) => pattern.test(s), `expected a string that matches ${pattern.toString()}`);
...
export const httpUrl = () =>
  tString().where((s) => patterns.httpUrl.test(s), `expected a valid http or https URL`);
